                        ...
                        @if($posts)
                            @foreach($posts as $post)
                                <tr>
                                    ...
                                    <td data-label="Photo"><img height="50" src="{{$post->photo ? asset('images/media/'. $post->photo->file) : asset('img/200x200.png') }}" alt=""><p class="mb-0 mt-2"><a href="{{ route('post.edit', $post->id) . '?language=' . request()->input('language')}}">{{clean( trans('niva-backend.edit') , array('Attr.EnableID' => true))}}</a></p></td>
